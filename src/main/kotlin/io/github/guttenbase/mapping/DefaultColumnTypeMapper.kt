...
open class DefaultColumnTypeMapper : ColumnTypeMapper {
  ...
  override fun mapColumnType(
    ...
    val columnDefinition = getMapping(sourceDatabaseType, targetDatabaseType, columnType)
    ...
    return if (columnDefinition != null) {
      ...
      columnDefinition.type + precision + notNull
    } else {
      getColumnType(columnMetaData, "") + notNull
    }
  }
   ...
   */
  protected fun getColumnType(columnMetaData: ColumnMetaData, optionalPrecision: String): String {
  ...
  protected fun createPrecisionClause(columnMetaData: ColumnMetaData, optionalPrecision: String): String {
    return when (columnMetaData.columnType) {
      Types.CHAR, Types.VARCHAR, Types.VARBINARY -> if (columnMetaData.columnTypeName.uppercase().contains("TEXT")) {
        ...
        val precision: Int = columnMetaData.precision
  ...
  fun getMapping(sourceDB: DatabaseType, targetDB: DatabaseType, columnType: String): ColumnDefinition? {
    val databaseMatrix = mappings[sourceDB]

    if (databaseMatrix != null) {
      val databaseMapping = databaseMatrix[targetDB]
