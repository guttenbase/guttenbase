    ...
    for (currentRow in 0 until numberOfRowsPerBatch) {
      ...
      for (columnIndex in 1..sourceColumns.size) {
        ...
        for (targetColumn in mapping.columns) {
          ...
          val sourceValue = columnMapping.sourceColumnType.getValue(rs, columnIndex, sourceColumn)
          ...
          val optionalCloseableObject = columnMapping.targetColumnType.setValue(
            insertStatement, targetColumnIndex++, targetColumn, targetValue
          )
    ...
    ColumnDataMappingTool(connectorRepository).getCommonColumnTypeMapping(sourceColumn, targetColumn)
      ?: throw IncompatibleColumnsException(
        """|
          ...
          |in table ${sourceColumn.container.tableName}
