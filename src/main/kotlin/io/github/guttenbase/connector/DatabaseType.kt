...
enum class DatabaseType(
  ...
  POSTGRESQL("", // Handled by column type, see below)
  ...
  DB2("IDENTITY($NEXT_VALUE, 1)"),

  SYBASE("IDENTITY", "CALL sa_reset_identity('$TABLE_NAME', 'DBA', $NEXT_VALUE - 1);"),

  ORACLE("GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH $NEXT_VALUE INCREMENT BY 1"),

  HSQLDB("GENERATED BY DEFAULT AS IDENTITY (START WITH $NEXT_VALUE, INCREMENT BY 1)"), // IDENTITY
  H2DB("GENERATED BY DEFAULT AS IDENTITY", "ALTER TABLE $TABLE_NAME ALTER COLUMN $COLUMN_NAME RESTART WITH $NEXT_VALUE;"), // AUTO_INCREMENT
  DERBY("GENERATED BY DEFAULT AS IDENTITY (START WITH $NEXT_VALUE, INCREMENT BY 1)")
  ;
  ...
  fun createColumnAutoincrementClause(column: ColumnMetaData): String {
    ...
    val nextValue = column.tableMetaData.maxId + 1

    return autoincrementColumnClause.replace(NEXT_VALUE, nextValue.toString())
  ...
  fun createColumnAutoincrementStatement(column: ColumnMetaData): String? {
    ...
    return if (autoincrementColumnStatement != null) {
      ...
      autoincrementColumnStatement.replace(NEXT_VALUE, nextValue.toString())
        .replace(TABLE_NAME, column.tableMetaData.tableName).replace(COLUMN_NAME, column.columnName)
    } else {
      null
    }
