 ...
 *
 * @author M. Dahm
 * Hint is used by [io.github.guttenbase.hints.CheckTableDataHint] Configure how many rows of tables shall be regarded when checking that data has been
 * transferred correctly and what algorithm should be used when checking a table.
 */
...
@JvmOverloads constructor(
	...
	fun checkTableData() {
		...
		val checkTableData = connectorRepository.hint<CheckTableData>(targetConnectorId)
		...
		val sourceDatabaseConfiguration1 = connectorRepository.getSourceDatabaseConfiguration(sourceConnectorId)
		val sourceDatabaseConfiguration2 = connectorRepository.getSourceDatabaseConfiguration(targetConnectorId)
		...
		for (sourceTable in sourceTables) {
			...
			if (checkTableData.checkBy(targetTable) == DATA_COMPARISON) {
				checkByDataComparison(targetTable, sourceTable, connection1, connection2)
			} else {
				checkTableDataByOrderedSelect(
					sourceConnectorId, connection1, sourceDatabaseConfiguration1, sourceTable, targetConnectorId,
					connection2, sourceDatabaseConfiguration2, targetTable, checkTableData.numberOfCheckedRows
				)
			}
		}
	...
	private fun checkByDataComparison(
	...
	private fun checkTableDataByOrderedSelect(
